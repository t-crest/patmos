...
class S4noc(n: Int, fifoDepth: Int) extends Module  {
  val io = new Bundle {
  ...
  val net = Module(new Network(dim, UInt(width = 32)))

  for (i <- 0 until n) {
    val ni = Module(new NetworkInterface(dim, fifoDepth, UInt(width = 32)))
