...
// Martin: SUPER, SUPER ugly workaround to have more than one UART
object Uart2 extends DeviceObject {
  var baudRate = -1
  var fifoDepth = -1
  ...
  def create(params: Map[String, String]) : Uart2 = {
    Module(new Uart2(CLOCK_FREQ, baudRate, fifoDepth))
  }

  trait Pins {
    val uart2Pins = new Bundle() {
      val tx = Bits(OUTPUT, 1)
      val rx = Bits(INPUT, 1)
    }
  }
...
class Uart2(clk_freq: Int, baud_rate: Int, fifoDepth: Int) extends CoreDevice() {

  override val io = new CoreDeviceIO() with Uart2.Pins
...
// Torur: Once more, same as before
object Uart3 extends DeviceObject {
  var baudRate = -1
  var fifoDepth = -1
  ...
  def create(params: Map[String, String]) : Uart3 = {
    Module(new Uart3(CLOCK_FREQ, baudRate, fifoDepth))
  }

  trait Pins {
    val uart3Pins = new Bundle() {
      val tx = Bits(OUTPUT, 1)
      val rx = Bits(INPUT, 1)
    }
  }
...
class Uart3(clk_freq: Int, baud_rate: Int, fifoDepth: Int) extends CoreDevice() {

  override val io = new CoreDeviceIO() with Uart3.Pins
