...
class AudioADC(AUDIOBITLENGTH: Int, FSDIV: Int) extends Module { //constants: from CONFIG parameters
  ...
  //states
  val sIdle :: sStart1 :: sStart2 :: sLeft :: sRight :: Nil = Enum(5)
  //state register
  val state = RegInit(init = sIdle)

  //Registers for outputs:
  val adcLrcReg = RegInit(init = 0.U(1.W))

  // register for read enable signal to buffer
  val readEnAdcReg = RegInit(init = 0.U(1.W)) // starts with read disabled
  ...
  val audioLReg = RegInit(VecInit.fill(AUDIOBITLENGTH)(0.U(1.W)))
  val audioRReg = RegInit(VecInit.fill(AUDIOBITLENGTH)(0.U(1.W)))
  val audioLRegO = RegInit(init = 0.U(AUDIOBITLENGTH.W))
  val audioRRegO = RegInit(init = 0.U(AUDIOBITLENGTH.W)) //connect registers to ouputs when conversion is not busy
  ...
  when(io.enAdcI === 1.U) {
    ...
    when((io.bclkI =/= bclkReg) && (io.bclkI === 0.U)) {
      ...
      .elsewhen((io.bclkI =/= bclkReg) && (io.bclkI === 1.U)) { //FSM for audio conversion
        switch(state) {
          is(sLeft) {
            readEnAdcReg := 0.U
