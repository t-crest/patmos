 ...
 */
class NI(n: Int, size: Int) extends Module {
  val io = new Bundle {
    val rx_local = new Channel()
    val tx_local = new Channel()
  ...
  io.rx_local.out.data := memTx.io.port.rdData
  // TDM schedule starts two cycles late for read data delay
  io.rx_local.out.valid := RegNext(valid, init = Bool(false))
  ...
  val validRx = io.rx_local.in.valid
  debug(validRx)
  ...
  memRx.io.port.wrData := io.rx_local.in.data
