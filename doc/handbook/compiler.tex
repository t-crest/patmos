...
Single-path code is an code generation technique that negates the need for WCET analysis using \code{platin}.
The resulting code will always have a constant execution time (CET), meaning measuring the execution time of the program using arbitrary input data is an alternative to using WCET bounds from \code{platin}.

\textit{Note: This section only works for version v2 of the compiler and above.}

\subsection{Enabling CET}

Using \code{patmos-clang} is the easiest way of using single-path code:

\begin{verbatim}
	pamtos-clang main.c -O2 -mpatmos-enable-cet -mpatmos-cet-functions=main
\end{verbatim}

The above command will generate a binary where the \code{main} function is using single-path code to have CET.
\code{-mpatmos-enable-cet} enables the compiler to produce CET functions, while \code{-mpatmos-cet-functions=main} tells it that the \code{main} function should use single-path code to achieve CET.
Any function called from \code{main}, recursively, will also become single-path.
...
\begin{itemize}
	...
	Therefore, \code{for}-loops can have a lower bound of 0, meaning the body may be skipped in the very first test of the loop condition.
	A \code{do-while}-loop, cannot have a lower bound of 0, since its body is always executed at least once.
	
	\item No recursion is allowed.
	\item No \code{goto} is allowed to result in a loop.
