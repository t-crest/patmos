...
\item \code{memory-areas}: The memory areas set up a mapping of address ranges to memories
  ...
  There may be at most one global code address space entry that refers to \code{main} memory. It must use either the
  \code{method-cache} or the \code{instruction-cache} if configured. All other code memory areas must refer to an ideal memory without using a cache.
