...
Patmos compiler.
In the mean time it is intended to collect design notes and discussions.
...
In the following we present an example configuration on a Linux/Ubuntu
system.\footnote{I used the 32-bit version to simplify Quartus installation.}
...
the complete build processes for the embedded version of
Hello World (a blinking LED) is as follows:

\todo{Test. I think I renamed helloworld.}
...
However, this involves quite many steps. Therefore, we suggest to do some \emph{manual}
...
utility, and the boot loader). This has to be only once.
...
blinking LED, from a C program. You can find the C source in \code{c/hello\_blinking.c}.
% make BOOTAPP=bootable-bootloader APP=helloworld tools synth comp config download

\martin{The following leads to 162 KB .bin file, which is a little bit too big for on-chip. Why is this?

make BOOTAPP=hello\_blinking comp gen synth config
}

\martin{But the following does the trick. Maybe we can simplify to two arguments: 
one for a ROM program and one for download application. Differentiate with make
targets and/or just if the symbols are set or not.}

\todo{maybe rename to a shorter name.}

\begin{verbatim}
make BOOTAPP=bootable-hello_blinking comp gen synth config
\end{verbatim}



\martin{I'm still a little bit confused when to use the APP and the BOOTAPP
thing. Tried BOOTAPP, but complained about something missing.}
