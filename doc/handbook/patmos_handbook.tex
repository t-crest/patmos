...
First, because Patmos uses Chisel as its hardware language,
which is built on top of Scala, we need to ensure we can install SBT. 
...
sudo apt install git openjdk-8-jdk gitk cmake make g++ texinfo flex bison \
  ...
  liblpsolve55-dev python zlib1g-dev gtkwave gtkterm scala autoconf libfl2 expect verilator
\end{verbatim}

Make sure to use Java 8 and remove any later Java version with
