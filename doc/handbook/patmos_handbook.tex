...
In the following we present an  Patmos build instructions on a Linux/Ubuntu
...
The same program can be executed by the emulator (the Chisel based simulator)
...
a C++ based emulator is generated, compiled, and executed. Again with options to
show the register content after each instruction.

Those two Patmos simulation, the software simulator and the Chisel based emulator,
...
When an application shall be generated to be executed from the on-chip ROM,
the name is set with the BOOTAPP variable.


\todo{Describe the ELF download flow.}
