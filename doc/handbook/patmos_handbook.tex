...
\paragraph{A More Complex Application}

Small example code (single C file) is distributed in folder \code{c} and
some subfolders (e.g., \code{bootable}, \code{bootloader}, \code{cmp},...)
and compiled with a \code{make comp APP=program}.

Larger applications, such as an operating system for Patmos such as
...
Following example builds an application

\begin{verbatim}
make app APP=bench
\end{verbatim}

which can the be downloaded as usual to an FPGA

\begin{verbatim}
make config download APP=bench
\end{verbatim}
...
\begin{lstlisting}[float,caption={A multicore blinking LED\label{fig:cmp:hello}}]
/*
