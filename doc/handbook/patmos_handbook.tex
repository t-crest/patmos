...
\todo{Is the above really the best paper on platin and the WCET analysis?}
In the section we will give a minimal ``Hello World'' example for WCET analysis
with platin.

First we can generate a hardware configuration description containing the
default configuration with:

\begin{verbatim}
platin pml-config --target patmos -o de2-115.pml
\end{verbatim}

\todo{Do we need this in the first place? Can we just use the defaults?}

The default configuration is written into \code{de2-115.pml} and
represents a single core Patmos on the Altera
DE2-115 board, with a memory that is configured for 4 32-bit bursts in 21
...
\todo{Is the default really correct?}
...
Details on WCET analysis with platin can be found in Section~\ref{sec:toolchain:platin}.
