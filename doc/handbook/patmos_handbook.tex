...
Optional package for documentation build: \code{doxygen}, which will install LaTeX as well.
...
For Aegean: \code{python33} and \code{py33-lxml}. Make a link from python3 to python3.3 as this is the way it is invoked.
...
\begin{verbatim}
/home/martin/t-crest/patmos/install/bin/patserdow -v /dev/ttyUSB0 \
   /home/martin/t-crest/patmos/tmp/hello_puts.elf
Port opened: true
Params set: true
...
When developing software for Patmos and using the boot loader, the FPGA
configuration needs only be synthesized once. We only need to compile
the application, configure the FPGA, and download the application:

\begin{verbatim}
make APP=hello_puts config download
\end{verbatim}

\paragraph{A Variation of the Setup}
Here an example of the individual steps to build the blinking LED C
hello world for a different FPGA board and splitting the process for
a mixed OS setup:
...
This split of the make commands is for demonstration of how to handle
two two operating systems (e.g., Mac OSX for compilation and Windows for synthesis).
...
to build a platform that contains a boot loader (similar to the single core version) and some
...
\begin{verbatim}
make make APP=hello_puts comp download
\end{verbatim}

This application is the some single core \emph{Hello World} application that
we used in Section~\ref{sec:elf:files}. However, here we just compiled the
application and downloaded it via the serial port. We synthesize and configure
the FPGA from within the Aegean project.
...
build of this platform needs some non-free tools and includes closed source code, we provide prebuilt
