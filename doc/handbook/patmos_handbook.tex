...
\begin{itemize}
  ...
  \item CFLr -- Control Flow with Registers or Short Immediate \\[2ex]
        \begin{bytefield}{32}
          \bitheader{0-31} \\
        ...
        \begin{bytefield}[leftcurly=.]{32}
          ...
          \end{leftwordgroup}
          \begin{leftwordgroup}{\parbox{18em}{CFLsi -- Control Flow with single short immediate}}
          \bitheader{0-31} \\
          \bitbox{1}{x} & \bitbox{4}{Pred} & \bitbox{4}{1100} & \bitbox{1}{d} &
          \bitbox{10}{\bitsunused} & \bitbox{5}{Imm} & \bitbox{3}{\bitsunused} &
          \bitbox{2}{11} & \bitbox{2}{Op}
          \end{leftwordgroup}
         \end{bytefield}
\end{itemize}
...
The variant \texttt{callsb} (call-and-split-block) of \texttt{call} sets the return information
so that the callee returns into a new subfunction after the call site.
It requires the new subfunction to be placed
\emph{immediately} after the delay slots of the \texttt{callsb} instruction,
i.e., the return subfunction needs to \emph{aligned to $4$ bytes}.
All other subfunctions can have arbitrary alignment, but should be
aligned to the burst size (typically $16$ bytes) for optimal performance.
\stefan{Maybe we fix the alignment for callsb to 16 bytes as well, but 
it should be independent of the burst size. Otherwise we need to know
and compile for the exact burst size in the compiler and adapt all .S files!}
...
\begin{table}[hb]
  ...
  \begin{tabular}{llll}
    ...
    11 & 0 & callsbnd & function call (absolute, with cache fill, non-delayed) \\
    11 & 1 & callsb   & delayed function call (absolute, with cache fill, delayed) \\
    \bottomrule
  \end{tabular}
...
\begin{table}[hb]
  ...
  \begin{tabular}{llll}
    ...
    11 & 0 & callsbnd & function call (indirect, with cache fill, non-delayed) \\
    11 & 1 & callsb   & function call (indirect, with cache fill, delayed) \\
    \bottomrule
  \end{tabular}
...
\begin{itemize}
  \item CFLsi -- Control Flow with single short immediate \\[2ex]
    \begin{bytefield}{32}
      \bitheader{0-31} \\
      \bitbox{1}{x} & \bitbox{4}{Pred} & \bitbox{4}{1100} & \bitbox{1}{d} &
      \bitbox{10}{\bitsunused} & \bitbox{5}{Imm} & \bitbox{3}{\bitsunused} &
...
\begin{table}[hb]
  ...
  \begin{tabular}{llll}
    ...
    Op & d & Name & Semantics \\
    \midrule
    00 & 0 & trap   & system call (via exception vector, with cache fill, non-delayed) \\
    \bottomrule
  \end{tabular}
  \caption{Control-flow operations with single register operand}
  \label{tab:cflrsops}
...
\stefan{I am not overly happy myself about making the CFLsi instruction a subformat
of CFLr, but it is the closest fit, and CFLri is already taken and CFLrsi seems like
a misnomer.}
...
\paragraph{Behavior -- callsb}

Perform a function call, filling the method cache if needed, and return
into a new subfunction.

The behavior and pseudo code of \texttt{callsb} and \texttt{callsbnd}
is similar to \texttt{call} and \texttt{callnd} respectively, except
that \texttt{\$srb} is set to $\texttt{PC} + 4$ and \texttt{\$sro}
is set to $0$.

\stefan{In case we would want to align the return subfunction, we would need
to set \texttt{srb} to $((\texttt{PC} + 0x10) \& ~0xF) + 4$.}
...
In the following we present the Patmos build instructions on a 64-bit Linux/Ubuntu
