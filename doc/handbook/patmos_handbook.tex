...
Note 4: % where are notes 1--3?
\martin{Why do we state so strongly that we want 4 words?}
Bursts are restricted to exactly four words (D1, D2, D3 and D4), or
some other constant which is a power of 2.  The address must be aligned.
...
\begin{table}[b!]
  ...
  \begin{tabular}{>{\tt}l<{}p{.6\textwidth}}
    \toprule
...
\begin{table}
  ...
  \begin{tabular}{>{\tt}l<{}>{\tt}r<{}p{.6\textwidth}}
    \toprule
...
\begin{table}
  ...
  \begin{tabular}{>{\tt}l<{}>{\tt}r<{}p{.59\textwidth}}
    \toprule
    \multicolumn{1}{l}{Option} & \multicolumn{1}{l}{Default} & Description \\
    \midrule
...
\begin{table}
  ...
  \label{tab:emulopts}
  \begin{tabular}{>{\tt}l<{}p{.66\textwidth}}
    \toprule
    \multicolumn{1}{l}{Option} & Description \\
    \midrule
    -h            & Print help \\
...
The general usage is \texttt{patserdow [-v] [-h] <port> <file>}. The
...
stderr. Output from the application is written to stdout; input to the
...
The FPGA is configured by calling \texttt{make config} in the
directory specified in environment variable \texttt{PATMOS\_HOME}.
When this variable is unset, \texttt{patex} uses the directory where
it was installed from. The environment variable \texttt{COM\_PORT}
sets the serial port for downloading. When this variable is unset,
\texttt{patex} uses the \texttt{COM\_PORT} variable from the Makefile
at the time of installation. The environment variable \texttt{TIMEOUT}
sets a timeout in seconds. By default, \texttt{patex} terminates
download and execution after 300 seconds.
