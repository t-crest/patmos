...
For the Argo NoC a schedule needs to be generated. This is done from within the
aegean repository with:
...
First, because Patmos uses Chissel as its hardware language, which is built on top of Scala, we need to ensure we can install SBT. 
Therefore, we start by enabling the distribution to download sbt:
...
$ sudo apt-get install git default-jdk gitk cmake make g++ texinfo flex bison\
  ...
  ruby-full liblpsolve55-dev python zlib1g-dev gtkwave gtkterm scala sbt
\end{verbatim}
...
Notice how the Windows 'C:' drive can be accesses from the Ubuntu terminal as 'mnt/c/'. 
To ensure Git accepts the private key, we need to restrict access to it:
...
begin building LLVM, navigate to '~/t-crest/llvm/build' and run:

\begin{verbatim}
$ make -j
\end{verbatim}

When the LLVM build is done, the build script can be rerun, and it will continue where it left off (now not needing to build the compiler).
