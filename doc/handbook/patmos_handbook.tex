...
\begin{verbatim}
brew install cmake boost libelf sbt doxygen expect svn
\end{verbatim}
...
@echo off
setlocal
rem Find USB devices with specified IDs
echo Searching for USB devices with IDs "046d:c52b"...
for /f "tokens=1,2 delims= " %%A in ('usbipd list ^| findstr 046d:c52b') do (
    echo Found device: %%A
    set USB_DEVICE_1=%%A
)
echo.

echo Searching for USB devices with IDs "09fb:6001"...
for /f "tokens=1,2 delims= " %%A in ('usbipd list ^| findstr 09fb:6001') do (
    echo Found device: %%A
    set USB_DEVICE_2=%%A
)
echo.

rem Bind the USB devices
if defined USB_DEVICE_1 (
    echo Binding USB device: %USB_DEVICE_1%
    usbipd bind --force -b %USB_DEVICE_1%
    echo.
)

if defined USB_DEVICE_2 (
    echo Binding USB device: %USB_DEVICE_2%
    usbipd bind --force -b %USB_DEVICE_2%
    echo.
)

rem Attach USB devices to WSL
if defined USB_DEVICE_1 (
    echo Attaching USB device: %USB_DEVICE_1%
    usbipd attach -w -b %USB_DEVICE_1%
    echo.
)

if defined USB_DEVICE_2 (
    echo Attaching USB device: %USB_DEVICE_2%
    usbipd attach -w -b %USB_DEVICE_2%
    echo.
)
