...
The correct Verilog file for two version of Patmos (core 0 that downloads and application and the other cores)
is built with the \emph{Aegean} tool. The 4 cores version are built with:
...
A 16 core version is available as well. The generated Verilog files are not different,
but the schedule for the Argo network is different (which is copied to
\code{t-crest/patmos/c/nocinit.c}).

The configuration of T-CREST with Bluetree and the TU/e memory controller
is available via a .zip file, exchanged via email to protect IP rights.
Therefore, they are not integrated in Aegean and
some manual code copying is needed.
