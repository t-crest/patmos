...
\begin{itemize}
  ...
  \item CFLi -- Control Flow with Immediate \nopagebreak\\[2ex]
        \begin{bytefield}{32}
          \bitheader{0-31} \\
          \bitbox{1}{x} & \bitbox{4}{Pred} & \bitbox{2}{10} & \bitbox{1}{D} & \bitbox{2}{Op} &
          \bitbox{22}{Immediate} \\
        \end{bytefield}
  ...
  \item CFLr -- Control Flow with Registers \\[2ex]
        \begin{bytefield}{32}
          \bitheader{0-31} \\
          \bitbox{1}{x} & \bitbox{4}{Pred} & \bitbox{5}{11000} &
...
\begin{itemize}
  \item CFLi -- Control Flow with Immediate \\[2ex]
    \begin{bytefield}{32}
      \bitheader{0-31} \\
      \bitbox{1}{x} & \bitbox{4}{Pred} & \bitbox{2}{10} & \bitbox{2}{D} & \bitbox{2}{Op} &
      \bitbox{22}{Immediate} \\
    \end{bytefield}
\end{itemize}
...
\begin{table}[hb]
  ...
  \begin{tabular}{lll}
    ...
    10 & brcf & branch (absolute, with cache fill) \\
    11 & ---  & unused \\
    \bottomrule
  \end{tabular}
...
\begin{table}[hb]
  ...
  \begin{tabular}{lll}
    ...
    0010 & brcf  & branch (indirect, with cache fill) \\
    0011 & ---   & unused \\
    \dots& \dots  & \dots \\
    1111 & ---   & unused \\
    \bottomrule
...
The software installation is started with a plain \code{setup.sh}. Then add
the bin directory of Quartus to your \$PATH.
...
\begin{verbatim}
make emulator
make comp APP=helloworld
bin/emulator tmp/helloworld.elf
\end{verbatim}
...
The LLVM compiler under OSX does not like \code{-Wno-narrowing},
which is needed for newer versions of \code{gcc}. Therefore, following
option is needed in the \code{build.cfg}: \code{GOLD\_CXXFLAGS=""}.
...
\begin{table}
  ...
  \begin{tabular}{llp{.6\textwidth}}
    ...
    \texttt{0xf?????80} & \texttt{vec<0>} & Address of exception handler 0, illegal operation \\
    \texttt{0xf?????84} & \texttt{vec<1>} & Address of exception handler 1, illegal memory access \\
    \ldots & \ldots & \ldots \\
    \texttt{0xf?????fc} & \texttt{vec<31>} & Address of exception handler 31 \\
...
\begin{table}
  ...
  \begin{tabular}{llp{.6\textwidth}}
    ...
    \midrule
    \texttt{exc}     & to Decode & Request injection of exception generated by pipeline \\
    \texttt{excAddr} & to Decode & Return address for exception generated by pipeline \\
    \texttt{intr}    & to Decode & Request injection of interrupt, must be ignored in delay slot \\
    \texttt{addr}    & to Decode & Address of exception handler to be executed \\
    \texttt{src}     & to Decode & Index of exception handler to be executed \\
    \cmidrule{1-3}

    \texttt{call}    & from Memory & Acknowledge execution of exception handler \\
    \texttt{ret}     & from Memory & Notify exception unit of return from exception handler \\
    \texttt{src}     & from Memory & Index of acknowledged or requested exception \\
    \texttt{exc}     & from Memory & Request triggering of exception \\
    \texttt{excAddr} & from Memory & Return address for requested exception \\
    \bottomrule
  \end{tabular}
...
are not used otherwise. Two special registers \texttt{sxb} (\texttt{s9}) and
\texttt{sxo} (\texttt{s10}) provide the return base and return offset for exceptions.
...
\texttt{sm} upon exceptions. Pipelined multiplications require special
attention too.
...
\bigskip
\begin{bytefield}[leftcurly=.]{32}
  \begin{leftwordgroup}{\parbox{12em}{CFLi -- Trap}}
    \bitheader{0-31} \\
    \bitbox{1}{x} & \bitbox{4}{Pred} & \bitbox{3}{10} & \bitbox{0}{1} & \bitbox{2}{11} &
    \bitbox{17}{\bitsunused} & \bitbox{5}{Func}
  \end{leftwordgroup} \\
\end{bytefield}
...
\bigskip
\begin{bytefield}[leftcurly=.]{32}
  \begin{leftwordgroup}{\parbox{12em}{CFLri -- \texttt{xret}}}
    \bitheader{0-31} \\
    \bitbox{1}{x} & \bitbox{4}{Pred} & \bitbox{5}{11000}
    \bitbox{18}{\bitsunused} & \bitbox{2}{00} & \bitbox{2}{01}
  \end{leftwordgroup} \\
\end{bytefield}
...
\item The interrupt has not been acknowledged, so signal
  \texttt{xu.intr} is still asserted. As the decode stage is
  technically in the delay slot of the previous interrupt instruction,
  it ignores the request. The previous interrupt instruction reaches
  the execute stage.
