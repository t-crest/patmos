...
Listing~\ref{lst:blink} shows the code for the embedded Hello World
...
\begin{lstlisting}[float,caption={A blinking LED\label{lst:blink}}]
/*
...
Currently, libnoc library doesn't support a receive message, therefore a receiver can know that the data has arrived through wait and polling to the specified address the data are axpected to arrive. Listing~\ref{lst:hello_multi} shows the code executed in the slave cores. The code for the allocation of memory in the SPM, the polling and the sending of message appears in Listing~\ref{lst:hello_multi}.
