...
but is intended as a stand alone document for anybody interested in learning to program the Argo Network-on-Chip.
...
This document presents the background required to efficiently program the Argo NoC.
...
\item[\code{qpd\_t * mp\_create\_qport( const unsigned int chan\_id, const direction\_t direction\_type, const coreid\_t remote, const size\_t msg\_size, const size\_t num\_buf)}]
...
\item[\code{spd\_t * mp\_create\_sport(const unsigned int chan\_id, const direction\_t direction\_type, const coreid\_t remote, const size\_t sample\_size)}]
