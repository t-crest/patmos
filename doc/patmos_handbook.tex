...
\subsection{Revised Branch Adressing Modes}

\wolf{I propose to change the branch adressing modes as shown in
...
(acesses to unmapped memory, undecodable instructions,
...
\item The exception unit requests an interrupt by asserting
  ...
  is ignored.
\item The signal \texttt{xu.intr} is still asserted, but the curreent
  instruction is not part os a branch-delay slot, so the decode stage
