...
However, I strongly oppose changing any of the indirect branches to anything else than absolute, it makes quite a mess 
...
\stefan{I know the hardware folks will hate me for this, but: we could allow stack cache access to happen
in the second pipeline, provided that there is no stack cache access in the first pipeline. 
The rationale for this is that we can still do spilling, maybe even data scratchpad access
in the second pipeline while the first pipeline is accessing main memory or doing control flow, 
without the need to double-clock the stack cache, and we can schedule mutually exclusive 
speculative execution paths (more) in parallel. 

I frankly do not know how much this will actually gain since usually the code contains either
main memory access or stack access, not a mixture of both, and we need a proper scheduler and if-converter first to
