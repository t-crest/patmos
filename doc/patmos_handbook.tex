...
make BOOTAPP=bootable-bootloader \\
  APP=helloworld tools synth comp \\
  config download
\end{verbatim}
...
\item[synth] create the verily code and synthesize for an FPGA
\end{description}

Here an example of individual steps to build the blinking LED C hello world:
\begin{verbatim}
make tools
make BOOTAPP=bootable-echo bootcomp
make BOOTAPP=bootable-echo synth
make config
\end{verbatim}

\todo{It would be nice to split the \code{synth} target into
Verilog generation/Scala execution and Quartus synthesis}

\todo{More words on individual \code{make} targets.}
