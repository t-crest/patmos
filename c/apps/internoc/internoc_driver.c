...
{
  int index = internoc_ip_to_corelinkid(config, dst_ip);
  return index >= 0 ? config.core_links[index].chanTx : NULL;
}

qpd_t * internoc_get_rxbuffer(InterNoCConfig config, const unsigned char src_ip[4])
{
  int index = internoc_ip_to_corelinkid(config, src_ip);
  return index >= 0 ? config.core_links[index].chanRx : NULL;
}
                          ...
                          unsigned char* data, unsigned short data_length)
{
  ...
  if(packet_channel != NULL)
  {
    ...
    packet_ptr->ip_head.length = sizeof(iphead_t) + sizeof(udphead_t) + data_length;
    ...
    packet_ptr->data = ((unsigned char * _SPM) (packet_channel->write_buf)) + sizeof(iphead_t) + sizeof(udphead_t);  //set the data address of the assigned SPM buffer after the IP/UDP head
...
void prin_core_channels(InterNoCConfig config)
{
  ...
  for(unsigned i = 0; i < config.core_links_num; i++){
      printf("link[%u] from %u, receive at buffer %p (size = %lu bytes)\n", i, config.core_links[i].chanRx->remote, config.core_links[i].chanRx->read_buf, MP_CHAN_BUF_SIZE);
  }
