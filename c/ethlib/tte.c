...
unsigned char frame[516] = {
    0xab, 0xad, 0xba, 0xbe, 0x0f, 0xa1, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x4C, 0x04, 0x00,
    ...
    0x00, 0x00, 0x30, 0x00, 0x00, 0x00, 0xc0, 0xe6, 0x94, 0x50, 0xfd, 0x7f, 0x00, 0x00, 0x00, 0xe6,
...
void tte_send_test_data(unsigned int tx_addr){ 
  ...
  //mem_iowr_byte(tx_addr + 11,num);
  mem_iowr_byte(tx_addr + 14,num);
  eth_mac_send(tx_addr, 90);
  return;
}
...
void tte_clock_tick(void) {
  ...
  LEDS ^= 1;
  timer_time += ((integration_period/5));
  arm_clock_timer(timer_time);
