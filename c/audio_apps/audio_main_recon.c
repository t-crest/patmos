...
#include "../../../reconfig/build/genesys2-audio-recon/bitstreams.c"

#include "../librticap/rticap.h"
#include "../librticap/rticap.c"

#define RR_MODES 4

#define PERIOD_CC 2083
#define SW_DSP_DELAY 64
#define HW_DSP_DELAY 10
...
void wait_delay (unsigned int del){
    int led_stat = *led_ptr;
    for (int i = 0; i < del; ++i){*led_ptr = led_stat;}
    return;
}

//Bypass the rr
void select_rr_bypass(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFF8) | 0; //000
    setGPIO(*gpio);
}

void select_rr_rr(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFF8) | 4; //000
    setGPIO(*gpio);
}

void select_rr_lp(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFF8) | 1; //000
    setGPIO(*gpio);
}

void select_rr_hp(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFF8) | 2; //000
    setGPIO(*gpio);
}

void select_rr_bp(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFF8) | 3; //000
    setGPIO(*gpio);
}

void enable_cpu_dsp(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFF7); //000
    setGPIO(*gpio);
}

void disable_cpu_dsp(unsigned int * gpio){
    *gpio = (*gpio | 0x00000008); //000
    setGPIO(*gpio);
}

void enable_input_buffer(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFDF); //000
    setGPIO(*gpio);
}

void disable_input_buffer(unsigned int * gpio){
    *gpio = (*gpio | 0x00000020); //000
    setGPIO(*gpio);
}

void enable_output_buffer(unsigned int * gpio){
    *gpio = (*gpio & 0xFFFFFFEF); //000
    setGPIO(*gpio);
}

void disable_output_buffer(unsigned int * gpio){
    *gpio = (*gpio | 0x00000010); //000
    setGPIO(*gpio);
}
...
int main() {
/*    int acc = 0;
    for (int i = 0; i < config_filter_pass_length; ++i)
        {
            acc = acc + config_filter_pass[i];
        }
    printf("CS1\n");
    for (int i = 0; i < config_filter_lp_length; ++i)
        {
            acc = acc + config_filter_lp[i];
        }
    printf("CS2\n");
    for (int i = 0; i < config_filter_hp_length; ++i)
        {
            acc = acc + config_filter_hp[i];

        }
    printf("CS3\n");
    for (int i = 0; i < config_filter_bp_length; ++i)
        {
            acc = acc + config_filter_bp[i];
        ...
        while(*keyReg != 3) {
            ...
            if( (*keyReg == 14) && (*keyReg != *keyReg_prev) ) {
                ...
                *FXp[*cmode_spm][FX_HERE[*cmode_spm]-1].last_init = 1;
                ...
                #ifdef NOC_RECONFIG
                disable_cpu_dsp(&gpio);
                disable_input_buffer(&gpio);
                wait_delay(SW_DSP_DELAY*PERIOD_CC);
                noc_sched_set(*cmode_spm);
                disable_output_buffer(&gpio);
                enable_input_buffer(&gpio);
                wait_delay(SW_DSP_DELAY*PERIOD_CC);
                enable_output_buffer(&gpio);
                enable_cpu_dsp(&gpio);
                #endif
            ...
            if( (*keyReg == 7) && (*keyReg != *keyReg_prev) ) {
                if(rr_mode == 0) {
                    rr_mode = RR_MODES - 1;
                }
                else {
                    rr_mode = (rr_mode-1) % RR_MODES;
                }
                printf("mode: %d_%d\n", rr_mode, *cmode_spm);
                //Incremnt HW mode
                select_rr_bypass(&gpio);
                disable_cpu_dsp(&gpio);
                ...
                switch(rr_mode) {
                    case 0:
                        for (int i = 0; i < config_filter_pass_length; i++)
                        {
                            rticap_reconspm_write32((unsigned int *) (i<<2), config_filter_pass[i]);
                        }
                    rticap_write_reg(RTICAP_BS_LENGTH_REG, (config_filter_pass_length-1)); //The length must be written with the subtraction of 1.
                    break;
                    case 1:
                        for (int i = 0; i < config_filter_lp_length; i++)
                        {
                            rticap_reconspm_write32((unsigned int *) (i<<2), config_filter_lp[i]);
                        }
                    rticap_write_reg(RTICAP_BS_LENGTH_REG, (config_filter_lp_length-1)); //The length must be written with the subtraction of 1.
                    break;
                    case 2:
                        for (int i = 0; i < config_filter_hp_length; i++)
                        {
                            rticap_reconspm_write32((unsigned int *) (i<<2), config_filter_hp[i]);
                        }
                    rticap_write_reg(RTICAP_BS_LENGTH_REG, (config_filter_hp_length-1)); //The length must be written with the subtraction of 1.
                    break;
                    case 3:
                        for (int i = 0; i < config_filter_bp_length; i++)
                        {
                            rticap_reconspm_write32((unsigned int *) (i<<2), config_filter_bp[i]);
                        }
                    rticap_write_reg(RTICAP_BS_LENGTH_REG, (config_filter_bp_length-1)); //The length must be written with the subtraction of 1.
                    break;
                    default : /* Optional */
                        printf("ERROR!\n");
                }
                rticap_write_reg(RTICAP_CONTROL_REG, START_RAM_STREAM_COMMAND);
                while(((rticap_read_reg(RTICAP_STATUS_REG)) & 0x0000FFFF)!=0x00000001){;}
                wait_delay(HW_DSP_DELAY*PERIOD_CC);
                select_rr_rr(&gpio);
                enable_cpu_dsp(&gpio);
            }
            if( (*keyReg == 11) && (*keyReg != *keyReg_prev) ) { //
                rr_mode = (rr_mode+1) % RR_MODES;
                printf("mode: %d_%d\n", rr_mode, *cmode_spm);
                //Incremnt HW mode
                select_rr_bypass(&gpio);
